...
`Dockerfile` packages the React frontend, FastAPI backend, Google ADK synthesis agent, and the official Grafana MCP binary in one service. `render.yaml` is a deployment blueprint for Render; configure only `GEMINI_API_KEY`, `PARALLEL_API_KEY`, `GRAFANA_URL`, and `GRAFANA_SERVICE_ACCOUNT_TOKEN` as encrypted service variables. The image does not contain `.env`.

For a hosted demo, set the three Grafana remote-write variables as encrypted host variables. The deployment then starts Prometheus beside the app, scrapes the instrumented `/metrics` endpoint, and remote-writes the clearly labeled demo samples to Grafana Cloud. The web service reads the resulting evidence through the bundled official Grafana MCP binary and does not require Docker Desktop or a local collector at runtime.
