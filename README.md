...
For a hosted demo to show fresh telemetry, run the Prometheus collector in a separate persistent environment using `monitoring/start-telemetry.ps1`, or deploy an equivalent collector that remote-writes to the same Grafana Cloud stack. The web service itself queries Grafana Cloud through the bundled official MCP binary; it does not require Docker Desktop at runtime.
